...
# ─Éß║úm bß║úo m├ú h├│a UTF-8 cho stdout tr├¬n mß╗ìi nß╗ün tß║úng
...
def find_student_file():
    """T├¼m ─æ╞░ß╗¥ng dß║½n file prompt_prototype.py ß╗ƒ c├íc th╞░ mß╗Ñc phß╗ò biß║┐n."""
    possible_paths = [
...
def check_file_exists(filename_pattern, search_dir="."):
    """Kiß╗âm tra sß╗▒ tß╗ôn tß║íi cß╗ºa mß╗Öt file (hß╗ù trß╗ú kiß╗âm tra phß║ºn mß╗ƒ rß╗Öng)."""
...
def check_workflow_diagram(search_dir="."):
    """Kiß╗âm tra file s╞í ─æß╗ô vß╗¢i nhiß╗üu ─æß╗ïnh dß║íng ß║únh/t├ái liß╗çu kh├íc nhau."""
...
def load_student_module(file_path):
    """Nß║íp ─æß╗Öng module python ─æß╗â kiß╗âm tra c├íc biß║┐n v├á h├ám."""
...
def run_autograder():
    ...
    # =========================================================================
    # PHß║ªN A: KIß╗éM TRA Sß╗░ Tß╗ÆN Tß║áI Cß╗ªA 4 FILE Nß╗ÿP B├ÇI (Tß╗æi ─æa 5.0─æ - 1.25─æ/file)
    # =========================================================================
    if run_a:
    ...
    # =========================================================================
    # PHß║ªN B: KIß╗éM TRA M├â NGUß╗ÆN PROMPT PROTOTYPE (Tß╗æi ─æa 5.0─æ - 1.0─æ/ti├¬u ch├¡)
    # =========================================================================
    if run_b:
        ...
        if not student_file_path:
            ...
            if student:
                # 1. Kiß╗âm tra SYSTEM_PROMPT (1.0─æ)
                try:
                    sys_prompt = getattr(student, "SYSTEM_PROMPT", "")
                    if not sys_prompt or "TODO:" in sys_prompt or "Write your strict" in sys_prompt:
                        all_code_passed = False
                ...
                # 2. Kiß╗âm tra evaluate_prompt() v├á Gemini SDK (1.0─æ)
                try:
                    eval_fn = getattr(student, "evaluate_prompt", None)
                ...
                # 3. Kiß╗âm tra ─æß╗ïnh ngh─⌐a Adversarial tests (1.0─æ)
                try:
                    tests = getattr(student, "ADVERSARIAL_TESTS", [])
                    if not isinstance(tests, list) or len(tests) < 2:
                        all_code_passed = False
                ...
                # 4. Kiß╗âm tra khß║ú n─âng thß╗▒c thi cß╗ºa script (1.0─æ)
                process_output = ""
                try:
                    result = subprocess.run(
                        [sys.executable, student_file_path], 
                ...
                # 5. Kiß╗âm tra kß║┐t quß║ú Assertions bß║úo vß╗ç ranh giß╗¢i (1.0─æ)
                if process_output:
    ...
    # =========================================================================
    # IN Kß║╛T QUß║ó V├Ç THIß║╛T Lß║¼P EXIT CODE
    # =========================================================================
    print("\n" + "="*60)
    ...
    # Tho├ít vß╗¢i m├ú lß╗ùi 1 nß║┐u thiß║┐u file hoß║╖c test code bß╗ï lß╗ùi trong phß║ºn ─æ╞░ß╗úc chß║íy
    should_fail = False
